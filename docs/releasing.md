...
5. creates the `vX.Y.Z` GitHub tag/release from the same commit and attaches the same distributions.

The release branch is only a trigger and can be deleted after a successful release.

A failed PyPI publish does not create the GitHub release.
